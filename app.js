...
const dockerOverlay = document.getElementById("docker-overlay");
const dockerClose = document.getElementById("docker-close");

function openDocker(e) {
  if (e) e.preventDefault();
  dockerOverlay.classList.add("open");
  dockerOverlay.setAttribute("aria-hidden", "false");
  lenis.stop();
}
function closeDocker() {
  dockerOverlay.classList.remove("open");
  dockerOverlay.setAttribute("aria-hidden", "true");
  lenis.start();
}
if (dockerTab) dockerTab.addEventListener("click", openDocker);
